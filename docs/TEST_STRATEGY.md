...
- Planned `pnpm verify:m6` adds strict assistant/provider/coordinator/IPC/preload/renderer contracts and one generated `MockAIProvider` Electron flow across status, natural-language search, set-plan/revision preview-confirm, grounded explanation, polling, cancellation, failure preservation, hashes, and cleanup. A separate explicit real Electron smoke must use exact SDK 0.147.0 with existing ChatGPT auth and record only redacted booleans/categories/counts. No API-key or visual gate is permitted.
