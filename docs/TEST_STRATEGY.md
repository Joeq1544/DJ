...
- `bash scripts/verify-m7.sh` passes 161 core tests, 248 desktop tests, strict typecheck/main/renderer builds, nine tests across eight isolated nonvisual Electron spec invocations, and exact verification of the composed personal app. It exercises every prior generated flow plus packaged import/analysis/discovery/personalization/mock-Copilot/set-export/diagnostics/backup/relaunch, immutable source hashes, runtime cleanup, and a copied-package missing-ffprobe case that cannot use an ambient replacement. Package verification checks the 630 MB arm64 artifact's fixed resources, exact versions, contained links, manifest hashes, component inventory/notices, and deep ad-hoc signature.
- `bash scripts/release/smoke-real-codex.sh` is the separate opt-in M7 authenticated gate. It launches the explicit packaged executable with a minimal `PATH` and temporary user-data path, proves exact SDK 0.147.0 existing-ChatGPT auth, bounded search and a second terminal plan request, cancellation, and cleanup, and records no response text.
...
Joe explicitly requested that all visual QA be skipped until the complete implementation is handed over. No M7 screenshot or visual/native-picker pass exists, and no automated result is interpreted as one. The deferred post-build hands-on period begins only when Joe chooses to test the finished artifact; it should record the target Mac/app build, input provenance without private contents, steps, outcomes, and limitations.

Personal XML/audio never enters Git. Real Codex checks are explicit, use existing auth, and record redacted outcomes without claiming behavior not observed. Real Rekordbox 7.2.14 import/export, native save/open appearance, subjective MIR/recommendation/set quality, and general visual polish remain unexecuted.
