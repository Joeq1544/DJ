...
bash -n scripts/setup-python.sh scripts/verify-m1.sh scripts/verify-m2.sh scripts/verify-m3.sh scripts/verify-m4.sh scripts/verify-m5.sh scripts/verify-m6.sh

for required_path in app/desktop/node_modules/.bin/electron app/desktop/node_modules/@openai/codex-sdk fixtures/rekordbox/phase0-library.xml fixtures/sets/m4-set.json; do
  if [[ ! -e "$required_path" ]]; then
    printf 'M6 verification prerequisite is missing: %s\n' "$required_path" >&2
    exit 1
  fi
done

sdk_version="$(node -p "require('./app/desktop/node_modules/@openai/codex-sdk/package.json').version")"
if [[ "$sdk_version" != "0.147.0" ]]; then
  printf 'Expected @openai/codex-sdk 0.147.0, found %s.\n' "$sdk_version" >&2
  exit 1
fi
