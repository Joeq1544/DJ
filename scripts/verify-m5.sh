...
bash -n scripts/setup-python.sh scripts/verify-m1.sh scripts/verify-m2.sh scripts/verify-m3.sh scripts/verify-m4.sh scripts/verify-m5.sh
...
for required_path in app/desktop/node_modules/.bin/electron fixtures/rekordbox/phase0-library.xml fixtures/sets/m4-set.json; do
  if [[ ! -e "$required_path" ]]; then
    printf 'M5 verification prerequisite is missing: %s\n' "$required_path" >&2
    exit 1
  fi
done
